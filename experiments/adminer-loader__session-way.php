...
// Init session the same way Adminer does it, but we need it here before it starts, to inject our config to session global.
// Then set SID const to prevent calling this code again there (it checks this const. it's not used anywhere, btw so expect it disappears suddenly in future Adminers)
// (copy of code from Adminer. wrapped to func to keep it isolated)
...
$_SESSION['pwds'][$ADM_driver][$dbConf['host'] . ':' . $dbConf['port']][$dbConf['user']] = $dbConf['password'];


// needed to be set to trigger authorization process in Adminer.
// (this method to do the autologin probably stopped to work in one of previous versions, so we use here the inject-into-session trick) 
//$_GET['username'] = $dbConf['username'];
